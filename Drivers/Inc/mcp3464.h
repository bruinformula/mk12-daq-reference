...
#ifndef MCP3464_EXT_VREF_VOLTS
#define MCP3464_EXT_VREF_VOLTS    3.3f
#endif

#define MCP3464_SCAN_CHANNEL_COUNT 8U
#define MCP3464_SAMPLE_RATE_WINDOW_MS 1000U
...
GPIO_TypeDef    		*csPort;
    ...
    volatile uint8_t  lastChannel;      /* Last channel ID seen in scan mode       */
    volatile uint8_t  readyMask;        /* Bitmask of channels updated since read  */
    volatile uint8_t  lastDrdyLevel;
    volatile uint8_t  lastRxStatus;
    volatile uint8_t  lastStartStatus;
    volatile int32_t  scanResult[MCP3464_SCAN_CHANNEL_COUNT];
    volatile float    scanVoltage[MCP3464_SCAN_CHANNEL_COUNT];
    volatile uint32_t channelSampleCount[MCP3464_SCAN_CHANNEL_COUNT];
    volatile float    channelSampleRate[MCP3464_SCAN_CHANNEL_COUNT];
    volatile uint32_t extiCount;
    volatile uint32_t armAttemptCount;
    volatile uint32_t armAcceptedCount;
    volatile uint32_t startKickCount;
    volatile uint32_t dmaStartOkCount;
    volatile uint32_t dmaStartFailCount;
    volatile uint32_t dmaCompleteCount;
    volatile uint32_t spiErrorCount;
    volatile uint32_t droppedBusyCount;
    volatile uint32_t droppedStoppedCount;
    volatile uint32_t lastSpiErrorCode;
    uint32_t          sampleRateLastUpdateMs;
    uint32_t          sampleRateLastCount[MCP3464_SCAN_CHANNEL_COUNT];
...
HAL_StatusTypeDef MCP3464_ArmRead(MCP3464_Handle *h);   /* Trigger one DMA read (call from EXTI) */
