...
void MLX90614_WriteReg(uint8_t devAddr, uint8_t regAddr, uint16_t data) {
	...
	HAL_I2C_Master_Transmit(&hi2c4, (devAddr << 1), i2cdata, 4, 0xFFFF);
	HAL_Delay(10);
...
uint16_t MLX90614_ReadReg(uint8_t devAddr, uint8_t regAddr, uint8_t dbg_lvl) {
	...
	uint32_t retries = 0;
	do {
		status = HAL_I2C_Mem_Read(&hi2c4, (devAddr<<1), regAddr, 1, in_buff, 3, 100);
		if (status == HAL_BUSY) {
			HAL_Delay(1);
		}
		retries++;
	} while (status == HAL_BUSY && retries < 100);

	if (status != HAL_OK) {
		return 0x0000; // Force 0 if read failed completely
	}
...
uint8_t MLX90614_ScanDevices(void) {
	HAL_StatusTypeDef result;
	for (int i = 1; i < 128; i++) {
		result = HAL_I2C_IsDeviceReady(&hi2c4, (uint16_t)(i << 1), 3, 5);

		if (result == HAL_ERROR) {
//			HAL_I2C_GetError(&hi2c4); /* error code available if needed */
			printf(".");
		}
...
void MLX90614_SetEmissivity(uint8_t devAddr, float emissivity)
{
    uint16_t target_e = (uint16_t)(emissivity * 65535.0f + 0.5f);
    uint16_t current_e = MLX90614_ReadReg(devAddr, MLX90614_EMISSIVITY, MLX90614_DBG_OFF);

    if (current_e != target_e) {
        MLX90614_WriteReg(devAddr, MLX90614_EMISSIVITY, target_e);
        HAL_Delay(100);
    }
}
