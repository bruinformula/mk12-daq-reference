...
static uint8_t sh1106_col_offset = 2;
...
uint8_t SH1106_Init(void) {
	...
	if (SDU_BOARD_ID == SDU_BOARD_RL || SDU_BOARD_ID == SDU_BOARD_RR) {
		SH1106_WRITECOMMAND(0xA0); //--set segment re-map 0 to 127 (flipped)
	} else {
		SH1106_WRITECOMMAND(0xA1); //--set segment re-map 0 to 127 (normal)
	}
	...
	if (SDU_BOARD_ID == SDU_BOARD_RL || SDU_BOARD_ID == SDU_BOARD_RR) {
		SH1106_WRITECOMMAND(0xC0); //Set COM Output Scan Direction (flipped)
		sh1106_col_offset = 2;     // Column offset for flipped mode
	} else {
		SH1106_WRITECOMMAND(0xC8); //Set COM Output Scan Direction (normal)
		sh1106_col_offset = 2;     // Column offset for normal mode
	}
...
void SH1106_UpdateScreen(void) {
	uint8_t m;
	uint8_t x = sh1106_col_offset;

	SH1106_Flush();
	
	for (m = 0; m < 8; m++) {
		SH1106_WRITECOMMAND(0xB0 + m);
		SH1106_WRITECOMMAND(0x00 | (x & 0x0F));
...
static char SH1106_Putc5x8(char ch, SH1106_COLOR_t color) {
	uint32_t row;
	uint32_t col;
	uint8_t glyph;
	...
	if (
		SH1106_WIDTH <= (SH1106.CurrentX + SH1106_CHAR_W) ||
		SH1106_HEIGHT <= (SH1106.CurrentY + 8)
	) {
		return 0;
	}

	for (col = 0; col < 5; col++) {
		glyph = font5x8[(uint8_t)ch - 32][col];
		for (row = 0; row < 8; row++) {
			if (glyph & (1U << row)) {
				SH1106_DrawPixel(SH1106.CurrentX + col, SH1106.CurrentY + row, color);
			} else {
				SH1106_DrawPixel(SH1106.CurrentX + col, SH1106.CurrentY + row, (SH1106_COLOR_t)!color);
			}
		}
	}

	for (row = 0; row < 8; row++) {
		SH1106_DrawPixel(SH1106.CurrentX + 5, SH1106.CurrentY + row, (SH1106_COLOR_t)!color);
	}

	SH1106.CurrentX += SH1106_CHAR_W;
	return ch;
}
...
char SH1106_Putc(char ch, FontDef_t* Font, SH1106_COLOR_t color) {
	...
	if (Font == &Font_5x8) {
		return SH1106_Putc5x8(ch, color);
	}
...
{
    uint8_t x = col + sh1106_col_offset;
    uint8_t buf[4] = {
...
{
	if (hi2c->Instance != SH1106_I2C->Instance) {
		return 0;
	}
